...
db = pymysql.connect('localhost', 'root', 'itsmecalk', 'Turner')
...
def query2exec():
    ...
    count = cursor.execute("INSERT INTO ITEMS(ITEM_NO,ITEM_DESCRIPTION) VALUES('%d','%s')" % (itemN, itemD))
...
def query11exec():
    ...
    amount_available = row1[0]
    cursor.execute("SELECT ORDER_NO FROM MADEOF WHERE ITEM_NO = %d" % (itemN))
    rows = cursor.fetchall()
    for row in rows:
        cursor.execute("SELECT ORDER_QTY FROM MADEOF AS MO, TOSUPPLY AS TS WHERE TS.ITEM_NO = MO.ITEM_NO")
        amount_available = amount_available - row.ORDER_QTY
